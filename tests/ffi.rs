...
use voluntary_servitude::ffi::{
    vsread_append, vsread_clear, vsread_destroy, vsread_iter, vsread_iter_destroy,
    vsread_iter_index, vsread_iter_len, vsread_iter_next, vsread_len, vsread_new,
};
...
fn mutability() {
    unsafe {
        ...
        assert_eq!(*(vsread_iter_next(iter) as *const i32), 1);
        data = 4;
        let _ = data;
        assert_eq!(*(vsread_iter_next(iter2) as *const i32), 4);
